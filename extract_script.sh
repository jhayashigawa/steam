...
filter_test()
{	# hardcoded first seen date for testing purposes. bug in ubuntu but not others?

	first_seen_date="2014-10-29"
	grep -v $first_seen_date all_first_seen_dates.csv > testtest.csv
}
...
temp_something_first_seen()
{	# go through database and add 'time0' to each entry using lookup
	# table (appids_list.csv)
	#
	
	ess stream steam_queries "*" "*" "aq_pp -f,+1,eok - -d \\
		s:query_date s:appid X X X X X s:discount_price X \\
		-cmb,+1 appids_list.csv s:appid i:time0 -filt 'time0 > 0'" \
		| head -n 20

}

stream_first_discount()
{	# Define a column spec to comb through steam_queries datastore
	# and add time0 column from lookup table (appids_list.csv). Then
	# for valid entries, add percent column that calculates discount
	# percent.
	...
	ess stream steam_queries "*" "*" "aq_pp -f,+1,eok - -d \\
		...
		-filt 'time0 > 0' -if -filt 'retail_price>0' \\
		-eval f:percent \\
		'(retail_price-sale_price)/retail_price)'\\
		 -endif \\
		-c query_date appid Title grade n_reviews \\
		retail_price sale_price time0 percent \\
		" | head -n 20	
}
